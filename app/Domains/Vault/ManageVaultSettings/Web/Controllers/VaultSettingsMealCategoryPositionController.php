...
class VaultSettingsMealCategoryPositionController extends Controller
{
    public function update(Request $request, string $vaultId, int $quickFactId)
    {
        $data = [
            ...
            'author_id' => Auth::id(),
            'vault_id' => $vaultId,
            'meal_category_id' => $quickFactId,
            'new_position' => $request->input('position'),
        ];

        $parameter = (new UpdateMealCategoryPosition())->execute($data);

        return response()->json([
            'data' => VaultSettingsIndexViewHelper::dtoMealCategory($parameter),
        ], 200);
    }
}
