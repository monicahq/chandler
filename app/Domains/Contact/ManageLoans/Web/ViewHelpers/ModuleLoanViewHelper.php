...
class ModuleLoanViewHelper
{
    ...
    public static function dtoLoan(Loan $loan, Contact $contact, User $user): array
    {
        ...
        return [
            ...
            'amount_full' => $loan->amount_lent ? MonetaryNumberHelper::format($user, $loan->amount_lent, optional($loan->currency)->code) : null,
            'amount_lent' => $loan->amount_lent ? MonetaryNumberHelper::formatValue($user, $loan->amount_lent, optional($loan->currency)->code) : null,
            'amount_lent_int' => $loan->amount_lent ? MonetaryNumberHelper::exchangeValue($loan->amount_lent, optional($loan->currency)->code) : null,
            'currency_id' => $loan->currency_id,
            'currency_name' => optional($loan->currency)->code,
