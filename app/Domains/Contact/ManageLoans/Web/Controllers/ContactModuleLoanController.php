...
class ContactModuleLoanController extends Controller
{
    ...
    public function update(Request $request, int $vaultId, string $contactId, int $loanId)
    {
        $loaners = collect($request->input('loaners'))->pluck('id');
        $loanees = collect($request->input('loanees'))->pluck('id');
    ...
    public function destroy(Request $request, int $vaultId, string $contactId, int $loanId)
    {
        $data = [
            'account_id' => Auth::user()->account_id,
            'author_id' => Auth::id(),
