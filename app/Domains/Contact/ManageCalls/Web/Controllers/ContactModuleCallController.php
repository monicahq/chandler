...
class ContactModuleCallController extends Controller
{
    ...
    public function update(Request $request, int $vaultId, string $contactId, int $callId)
    {
        $carbonDate = Carbon::parse($request->input('called_at'));
        $answered = false;
        $whoInitiated = 'me';
    ...
    public function destroy(Request $request, int $vaultId, string $contactId, int $callId)
    {
        $data = [
            'account_id' => Auth::user()->account_id,
            'author_id' => Auth::id(),
