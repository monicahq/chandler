...
class ContactModuleReminderController extends Controller
{
    ...
    public function update(Request $request, int $vaultId, string $contactId, int $reminderId)
    {
        if ($request->input('choice') == 'full_date') {
            $carbonDate = Carbon::parse($request->input('date'));
            $day = $carbonDate->day;
    ...
    public function destroy(Request $request, int $vaultId, string $contactId, int $reminderId)
    {
        $data = [
            'account_id' => Auth::user()->account_id,
            'author_id' => Auth::id(),
