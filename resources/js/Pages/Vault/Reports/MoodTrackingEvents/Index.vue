      ...
      <div class="mx-auto max-w-3xl px-2 py-2 sm:px-6 sm:py-6 lg:px-8">
        ...
        <div class="flex justify-center">
          <div v-for="month in data.months" :key="month.id" class="mb-6 flex flex-col text-center">
            <h2 class="mb-1 font-mono text-sm font-bold ltr:mr-2 rtl:ml-2">{{ month.month_word }}</h2>

            <div v-for="day in month.days" :key="day.id">
              <div
                v-if="day.event"
                class="inline-block h-4 w-4 rounded-full ltr:mr-2 rtl:ml-2"
                :class="day.event.hex_color" />
              <div v-else class="inline-block h-4 w-4 rounded-full ltr:mr-2 rtl:ml-2">
                <div class="inline-block h-4 w-4 rounded-full bg-slate-100 ltr:mr-2 rtl:ml-2" />
              </div>
            </div>
          </div>
        </div>
