...
<template>
  <div class="mb-12">
    ...
    <div class="sm:flex items-center justify-between mb-3 sm:mt-0 mt-8">
      ...
      <pretty-span v-if="!addUserModalShown" @click="showAddUserModal" :text="'Add a user'" :icon="'plus'" />
      <pretty-span v-if="addUserModalShown && localUsersInAccount.length == 0" @click="addUserModalShown = false" :text="'Cancel'" />
    </div>
        ...
        <li v-for="user in localUsersInVault" :key="user.id" class="border-b border-gray-200 hover:bg-slate-50 item-list">
          <div class="flex justify-between items-center px-5 py-2">
            ...
            <ul v-if="user.id != layoutData.user.id" class="text-sm">
              <li class="mr-4 cursor-pointer inline text-sky-500 hover:text-blue-900" @click="rename(user)">Change permission</li>
