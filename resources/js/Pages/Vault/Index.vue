  ...
  <Layout title="Dashboard" :user="user">
    <main class="sm:mt-24 relative">
      <div class="max-w-4xl mx-auto px-2 py-2 sm:py-6 sm:px-6 lg:px-8">
        ...
        <!-- blank state -->
        <div v-if="!data.vaults" class="bg-white border border-gray-200 rounded-lg mb-6 p-5">
          ...
          <div class="text-center">
            <pretty-link :href="data.url.vault.new" :text="'Create a vault'" :icon="'plus'" :classes="'mr-3'" />
          </div>
        </div>

        <div v-if="data.vaults">
          ...
          <div class="vault-list grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div v-for="vault in data.vaults" :key="vault.id" class="bg-white border border-gray-200 rounded-lg">
              <div class="grid vault-detail">
                <p class="border-b border-gray-200 text-lg p-3 font-medium">
                  {{ vault.name }}
                </p>
