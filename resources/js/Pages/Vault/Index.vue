...
<template>
  <layout title="Dashboard" :layout-data="layoutData">
    <main class="relative mt-16 sm:mt-24">
      <!-- blank state -->
      <div v-if="data.vaults.length == 0" class="mx-auto max-w-md px-2 py-2 sm:py-6 sm:px-6 lg:px-8 mb-6 ">
        <div class="rounded-t-lg border-t border-l border-r border-gray-200 bg-white p-5">
          ...
          <p class="mb-3">
            To start, you need to create a vault.
          </p>
          <div class="text-center mb-3">
            <pretty-link :href="data.url.vault.create" :text="'Create a vault'" :icon="'plus'" />
          </div>
        </div>
        <div class="border border-gray-200 p-5 rounded-b-lg">
      ...
      <div v-if="data.vaults.length > 0" class="mx-auto max-w-4xl px-2 py-2 sm:py-6 sm:px-6 lg:px-8">
        <div class="mb-10 sm:mb-6 sm:flex items-center justify-between">
          <h3 class="dark:text-slate-200 sm:mb-0 mb-3">All the vaults in the account</h3>
          <pretty-link :href="data.url.vault.create" :text="'Create a vault'" :icon="'plus'" class="md:w-auto w-full" />
        </div>
